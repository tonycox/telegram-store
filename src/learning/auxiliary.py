...
def remove_grammar(review):
    ...
    for sentence in sentences:
        if sentences.strip():
            ...
            result_review.append(traverse_tree(result))
...
def index():
    ...
    matrix = vectorizer.fit_transform(data['clean_sum'])
    print("tf-idf matrix :\n", matrix)
    kmeans = KMeans(n_clusters=no_of_clusters, n_jobs=-1)

    data['cluster_num'] = kmeans.fit_predict(matrix)
...
def phrase_in(x, phrase):
    return True if x in phrase else None
    # if phrase in x:
    #     return True
    # else:
    #     return None
